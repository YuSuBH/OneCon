...
export const makeRevision = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!message || message.trim() === "") {
      return res.status(401).json({ message: "Please provide a valid prompt" });
    }
    ...
    await prisma.conversation.create({
      data: {
        role: "assistant",
        content: `I have updated your website! You can preview the changes below.`,
        projectId,
      },
    });
  ...
  try {
    ...
    await prisma.conversation.create({
      data: {
        role: "assistant",
        content:
          "I have rolled back your website to selected version. You can now preview it",
        projectId,
      },
    });
...
// Controller function to Get project code for preview
...
// Controller function to Get all published project
